// File Directory: src/pages/MoviesPage.jsx
...
function MoviesPage() {
  ...
  // Use the search hook which returns paginated results
  const { data, loading, error, query, setQuery, page, setPage } = useOmdbSearch(searchTerm, 1);
  ...
  const displayed = movies.slice((uiPage - 1) * ITEMS_PER_PAGE, uiPage * ITEMS_PER_PAGE);
  ...
  return (
    <div>
      ...
      <div className="container mx-auto px-6 md:px-12 py-8 min-h-[70vh]">
        <h1 className="text-4xl font-extrabold mb-6 text-blue-400">All Movies</h1>
        <p className="text-gray-400 mb-8">Search and filter through our extensive library of films.</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 gap-6">
          ...
          {/* If no query yet, prompt the user */}
          {!query && !loading && (
            <div className="col-span-full text-center text-gray-400">Enter a search term above to find movies.</div>
          )}
+
          {displayed.map(movie => (
            <Mcard key={movie.id} movie={movie} onClick={() => console.log(`Open ${movie.title}`)} />
          ))}
        </div>
        ...
        <div className="flex justify-center items-center gap-4 mt-8">
          ...
          <button 
            ...
            onClick={() => {
              ...
              if (neededItems > movies.length && movies.length < total) {
        ...
        {/* Suggestions section (movies-only) */}
        <div className="mt-12">
          <Suggestions type="movie" />
        </div>
